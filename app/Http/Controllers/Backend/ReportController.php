...
class ReportController extends Controller
{
    ...
    public function getSalesReport()
    {
        ...
        if (auth()->user()->isAdmin()) {
            ...
            $acadmies = User::role('academy')->with('academy')->first();
            $academy = $acadmies->id;
    ...
    public function getCourseData(Request $request)
    {
        ...
        if (auth()->user()->hasRole('academy')) {
            ...
            $academyCourses = Course::withoutGlobalScope('filter')->where('academy_id', $academy)->pluck('id')->toArray();
            $courses = array_merge($courses, $academyCourses);
            $courses = Course::withoutGlobalScope('filter')->pluck('id');
        }
        $course_orders = OrderItem::whereHas('order', function ($q) {
            ...
            ->select('item_id', 'order_items.updated_at', DB::raw('count(*) as orders, sum(orders.amount) as earnings'))
            ->groupBy(['item_id', 'updated_at'])
            ->get();
//        dd($course_orders);
        return DataTables::of($course_orders)
            ...
            ->editColumn('course', function ($q) {
                ...
                return $link;
            })->editColumn('academy', function ($q) {
                $course = Course::withoutGlobalScope('filter')->with('academy')->find($q->item_id);
                $academy = '';
                if ($course->academy && $course->academy->user) {
                ...
                return date('Y-M-D h:i a', strtotime($q->updated_at));
            ...
            ->rawColumns(['course', 'academy', 'updated_at','teachers'])
            ->make();
    }
