...
// dd($chapter_lessons);
    ...
    {
        if ($request->type == 'popular') {
            $courses = Course::withoutGlobalScope('filter')->with('teachers')->where('published', 1)->where('popular', '=', 1);

        } else if ($request->type == 'trending') {
            $courses = Course::withoutGlobalScope('filter')->with('teachers')->where('published', 1)->where('trending', '=', 1);

        } else if ($request->type == 'featured') {
            $courses = Course::withoutGlobalScope('filter')->with('teachers')->where('published', 1)->where('featured', '=', 1);

        } else {
            $courses = Course::withoutGlobalScope('filter')->with(['teachers'])->where('published', 1);
        }
        if ($request->maxPrice) {
        ...
        foreach ($courses->items() as $course) {
            $html .= '<div class="col-xl-3 col-lg-3 col-md-3 col-12 mb-2"><div class="best-course-pic-text relative-position">
            ...
            $html .= '</ul> </div><span class="ml-1  rate">4.4 (222)</span></div>
                            </div>
            ...
            if (auth()->check() && (auth()->user()->hasRole('student')) && (Cart::session(auth()->user()->id)->get($course->id))) {
                $html .= '<button type="submit"
                                                                    class="btn btn-block btn-info ">   ' . trans('labels.frontend.course.add_to_cart') . '
                                                                <i class="fa fa-shopping-bag ml-1"></i>
                                                            </button>';
            } elseif (!auth()->check()) {
                if ($course->free == 1)
                    $html .= '<a class="btn btn-block btn-info"
                                                                       href="' . route('login.index') . '">' . trans('labels.frontend.course.get_now') . '
                                                                        <i class="fas fa-caret-right"></i>
                                                                    </a>';
                else {
                    $html .= '<a class="btn btn-block btn-info"
                                                                       href = "' . route('login.index') . '"> ' . trans('labels.frontend.course.add_to_cart') . '
                                                                        <i class="fa fa-shopping-bag" ></i >
                                                                    </a >';
                }
            } elseif (auth()->check() && (auth()->user()->hasRole('student'))) {
                if ($course->free == 1) {
                    $html .= '<form action = "' . route('cart.getnow') . '" method = "POST">
                    ...
                    $html .= '<form action = "' . route('cart.addToCart') . '"  method = "POST" >
                                                                         <input type="hidden" name="csrf-token" content="' . csrf_token() . '">
                                                                    <input type = "hidden" name = "course_id"
                                                                           value = "' . $course->id . '" />
                                                                    <input type = "hidden" name = "amount"
