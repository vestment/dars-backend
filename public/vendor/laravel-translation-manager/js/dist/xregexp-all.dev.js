...
(function (root, definition) {
  ...
  if (typeof define === 'function') {
    define(definition); // CommonJS
  } else if ((typeof exports === "undefined" ? "undefined" : _typeof(exports)) === 'object') {
    self = definition(); // Use Node.js's `module.exports`. This supports both `require('xregexp')` and
    // `require('xregexp').XRegExp`

    ((typeof module === "undefined" ? "undefined" : _typeof(module)) === 'object' ? module.exports = self : exports).XRegExp = self; // <script>
  } else {
    // Create global
  ...
  var XRegExp = function (undefined) {
    ...
    function augment(regex, captureNames, xSource, xFlags, isInternalOnly) {
      ...
      if (regex.__proto__) {
        regex.__proto__ = _self.prototype;
      } else {
    ...
    function getNativeFlags(regex) {
      return hasFlagsProp ? regex.flags : // Explicitly using `RegExp.prototype.toString` (rather than e.g. `String` or
    ...
    function isQuantifierNext(pattern, pos, flags) {
      ...
      /^(?:\s+|#.*|\(\?#[^)]*\))*(?:[?*+]|{\d+(?:,\d*)?})/ : // Ignore any leading inline comments
    ...
    function prepareOptions(value) {
      ...
      if (isType(value, 'String')) {
        _self.forEach(value, /[^\s,]+/, function (match) {
          options[match] = true;
        });
    ...
    function runTokens(pattern, flags, pos, scope, context) {
      ...
      while (i--) {
        ...
        match = _self.exec(pattern, t.regex, pos, 'sticky');
    ...
    _self = function self(pattern, flags) {
      ...
      if (_self.isInstalled('astral') && flags.indexOf('A') === -1) {
      ...
      if (!patternCache[pattern][flags]) {
        ...
        appliedFlags = result.flags; // Use XRegExp's tokens to translate the pattern to a native regex pattern.
        ...
        while (pos < appliedPattern.length) {
          ...
          if (result) {
            ...
            token = _self.exec(appliedPattern, nativeTokens[scope], pos, 'sticky')[0];
            output += token;
            pos += token.length;
    ...
    _self.addToken = function (regex, handler, options) {
      options = options || {};
    ...
    _self.cache = function (pattern, flags) {
      ...
      return regexCache[pattern][flags] || (regexCache[pattern][flags] = _self(pattern, flags));
    }; // Intentionally undocumented


    _self.cache.flush = function (cacheName) {
    ...
    _self.exec = function (str, regex, pos, sticky) {
      ...
      r2.lastIndex = pos = pos || 0; // Fixed `exec` required for `lastIndex` fix, named backreferences, etc.
    ...
    _self.forEach = function (str, regex, callback) {
      var pos = 0,
          i = -1,
          match;
    ...
    _self.match = function (str, regex, scope) {
    ...
    _self.replace = function (str, search, replacement, scope) {
      ...
      if (isRegex) {
        ...
        s2 = new RegExp(_self.escape(String(search)), 'g');
    ...
    _self.replaceEach = function (str, replacements) {
      var i, r;

      for (i = 0; i < replacements.length; ++i) {
        r = replacements[i];
        str = _self.replace(str, r[0], r[1], r[2]);
      }
    ...
    _self.union = function (patterns, flags) {
      ...
      for (i = 0; i < patterns.length; ++i) {
        ...
        if (_self.isRegExp(pattern)) {
          ...
          captureNames = pattern[REGEX_DATA] && pattern[REGEX_DATA].captureNames || []; // Rewrite backreferences. Passing to XRegExp dies on octals and ensures patterns
          // are independently valid; helps keep this simple. Named captures are put back

          output.push(nativ.replace.call(_self(pattern.source).source, parts, rewrite));
        } else {
  ...
  (function (XRegExp) {
    ...
    function deanchor(pattern) {
      var leadingAnchor = /^\^/,
          trailingAnchor = /\$$/; // Ensure that the trailing `$` isn't escaped
  ...
  (function (XRegExp) {
    'use strict'; // Storage for Unicode data
